...
export function EntryForm({ escopo = "empresa" }: { escopo?: EntityKind }) {
  const search = useSearch({
    from: escopo === "franquia" ? "/_authenticated/lancamentos-franquia/novo" : "/_authenticated/lancamentos/novo",
  });




  const scopedIndicators = useStore((s) => s.indicators.filter((i) => i.entity_scope === escopo));
  ...
  const [indId, setIndId] = useState<string>((search as { indicator?: string }).indicator ?? scopedIndicators[0]?.id ?? "");
  const ind = scopedIndicators.find((i) => i.id === indId);
  ...
  useEffect(() => {
    if (!indId && scopedIndicators.length > 0) setIndId((search as { indicator?: string }).indicator ?? scopedIndicators[0].id);
  }, [indId, scopedIndicators, search]);
  ...
  const save = async (status: "rascunho" | "registrado") => {
    ...
    try {
      ...
      try {
        ...
        console.error("[entry-form:refresh]", refreshErr);
      }
      toast.success(status === "registrado" ? "Lançamento registrado" : "Rascunho salvo");
      navigate({ to: escopo === "franquia" ? "/lancamentos-franquia" : "/lancamentos" });
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error("[entry-form:save]", err);
          ...
          <CardContent className="space-y-3">
            <Field label="Indicador">
              <Select value={indId} onValueChange={setIndId}>
                ...
                <SelectContent>{scopedIndicators.map((i) => <SelectItem key={i.id} value={i.id}>{i.name}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            {indicatorHasCompany && (
              <Field label="Empresa">
