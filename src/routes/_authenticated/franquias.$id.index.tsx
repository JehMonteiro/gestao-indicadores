...
function FranchiseDetail() {
  ...
  const unitIndicators = indicators.filter(
    (i) => i.entity_scope === "franquia" && (i.entity_id === id || i.franchise_id === id),
  );
  return (
    <div>
      ...
      <Card className="mt-4">
        <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0">
          ...
          <Button size="sm" asChild>
            <Link to="/franquias/$id/indicadores/novo" params={{ id }}>Novo indicador</Link>
          </Button>
        </CardHeader>
        <CardContent className="space-y-2">
